...
public interface ITestRepo extends JpaRepository<Test, UUID> {
    @Query("SELECT t FROM Test t WHERE " +
            ...
            "AND (:userId IS NULL OR t.userInfo.id = :userId) " +
    ...
    Page<Test> searchTest(String keyword, UUID userId, LocalDateTime dateStart, LocalDateTime dateEnd, UUID createdBy, Pageable pageable);
